        ...
        await readSseStream(response.body, ({ data, event: eventName }) => {
            ...
            try {
                ...
                if (event.type === 'reasoning_delta' && event.delta) {
                    // The model is thinking — stream its reasoning into the
                    // collapsible above the reply bubble.
                    if (!target.thinking) {
                        target.thinking = '';
                    ...
                    // The answer started — freeze the thinking timer and
                    // collapse the reasoning out of the way.
