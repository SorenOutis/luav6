...
import { BookOpen, Folder, LayoutGrid, Menu, Search, Moon, Sun, Bell, Zap, Shield, TrendingUp } from 'lucide-vue-next';
...
}>(() => (page.props.notifications as { unreadCount: number; items: HeaderNotification[] } | undefined) ?? {
    unreadCount: 0,
    items: [],
});
...
const markNotificationAsRead = (notificationId: string, href?: string | null) => {
                        ...
                        <div class="flex items-center gap-2 sm:gap-4 ml-2">
                            <DropdownMenu>
                                <DropdownMenuTrigger :as-child="true">
                                    <button
                                        class="relative inline-flex items-center justify-center rounded-md p-1.5 sm:p-2 text-sm font-medium transition-colors hover:bg-neutral-100 dark:hover:bg-neutral-800"
                                        aria-label="Open notifications"
                                    >
                                        <Bell class="h-4 w-4 sm:h-5 sm:w-5" />
                                        <span
                                            v-if="notifications.unreadCount > 0"
                                            class="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-primary px-1 text-[9px] font-black leading-4 text-primary-foreground"
                                        >
                                            {{ notifications.unreadCount > 9 ? '9+' : notifications.unreadCount }}
                                        </span>
                                    </button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="end" class="w-80 border-border/80 bg-background/95 p-0 shadow-2xl backdrop-blur-xl supports-[backdrop-filter]:bg-background/90">
                                    ...
                                    <div v-if="notifications.items.length > 0" class="max-h-96 overflow-y-auto p-2">
                                        <button
                                            v-for="notification in notifications.items"
                                            :key="notification.id"
                                            class="flex w-full items-start gap-3 rounded-lg px-2 py-2 text-left transition-colors hover:bg-accent/70"
                                            @click="markNotificationAsRead(notification.id, notification.href)"
                                        >
                                            <div class="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-primary/10">
                                                <img v-if="notification.image" :src="notification.image" :alt="notification.title" class="h-full w-full object-cover" />
                                                <component v-else :is="notificationIcon(notification.icon)" class="h-4 w-4 text-primary" />
                                            </div>
                                            <div class="min-w-0 flex-1">
                                                ...
                                                <p v-if="notification.message" class="mt-0.5 line-clamp-2 text-[11px] text-muted-foreground">
                                                    {{ notification.message }}
                                                </p>
                                                <div class="mt-1 flex items-center justify-between gap-2">
                                                    <span class="truncate text-[10px] uppercase tracking-wide text-muted-foreground/80">
                                                        {{ notification.meta ?? notification.type }}
                                                    </span>
                                    ...
                                    <div v-else class="bg-background/80 px-4 py-6 text-center">
                                        ...
                                        <p class="mt-1 text-xs text-muted-foreground">XP, level-up, and badge alerts will appear here.</p>
                                    </div>
                                </DropdownMenuContent>
                            </DropdownMenu>
