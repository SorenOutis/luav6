...
class ListExamSubmissions extends ListRecords
{
    ...
    public function getTabs(): array
    {
        ...
        foreach (Section::query()->orderBy('name')->get() as $section) {
            ...
            $tabs['section_'.$sectionId] = Tab::make($section->name)
                ->modifyQueryUsing(fn (Builder $query) => $query->whereHas(
                    'exam',
                    fn (Builder $examQuery) => $examQuery->where('section_id', $sectionId),
                ));
        }
        ...
        $tabs['no_section'] = Tab::make('No section')
            ->modifyQueryUsing(fn (Builder $query) => $query->whereHas(
                'exam',
                fn (Builder $examQuery) => $examQuery->whereNull('section_id'),
            ));
    ...
    {
        return 'Choose a section, then review each exam and its student submissions.';
    }
}
