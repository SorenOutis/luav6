...
const contextTips: ContextAwareTip[] = [
    {
        condition: (courses) =>
            courses.some((c) => c.progress >= 75 && c.progress < 100),
        generate: (courses) => {
            const near = courses.filter(
                (c) => c.progress >= 75 && c.progress < 100,
            );
            if (near.length === 1) {
    ...
    {
        condition: (courses) =>
            courses.length > 0 && courses.every((c) => c.progress >= 100),
        generate: (courses) => {
    ...
    {
        condition: (courses) =>
            courses.some((c) => c.completedLessons > 0 && c.progress < 100),
        generate: (courses) => {
            const best = courses.reduce((a, b) =>
                a.completedLessons > b.completedLessons ? a : b,
            );
...
const pickRandomTip = () => {
    ...
    const contextEligible = contextTips.filter((t) =>
        t.condition(props.courses),
    );
    ...
    do {
        if (tryContext) {
            const pickedContext =
                contextEligible[
                    Math.floor(Math.random() * contextEligible.length)
                ];
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
            ...
            >
                ...
                >
                    <div
                        v-for="i in 3"
                        :key="i"
                        class="h-72 animate-pulse rounded-xl border border-border/10 bg-card/30"
                    ></div>
                </div>
            </div>
        </template>
        ...
        <template v-if="isBooted">
            ...
            >
                ...
                >
                    <div
                        class="relative overflow-hidden rounded-2xl border border-border/20 bg-gradient-to-br from-primary/[0.04] via-muted/[0.02] to-transparent p-6 md:p-8"
                    >
                        <!-- Decorative glow -->
                        <div
                        ...
                        <div class="relative flex flex-col gap-4">
                            ...
                            <p
                                class="text-lg leading-relaxed font-medium tracking-tight text-foreground/90 md:text-2xl md:leading-snug"
                            >
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div
                                    class="h-px flex-1 bg-gradient-to-r from-primary/20 via-border/20 to-transparent"
                                ></div>
                                <button
                                    @click="pickRandomTip"
                    ...
                    >
                        ...
                        <div class="relative max-w-md flex-1">
                            ...
                            <input
                                ...
                                type="text"
                                placeholder="Search courses..."
                                class="w-full rounded-xl border border-border/40 bg-background/60 py-2.5 pr-9 pl-10 text-sm transition-all outline-none placeholder:text-muted-foreground/40 focus:border-primary/30 focus:ring-2 focus:ring-primary/10"
                            />
                            <button
                                v-if="searchQuery"
                                @click="searchQuery = ''"
                                class="absolute top-1/2 right-2.5 -translate-y-1/2 rounded-lg p-0.5 text-muted-foreground/40 transition-colors hover:text-foreground"
                            >
                                <X class="h-4 w-4" />
                            </button>
                        </div>
                        ...
                        <div class="flex items-center gap-2">
                            <!-- Status Filter -->
                            <div
                                class="flex scrollbar-none overflow-x-auto rounded-xl border border-border/40 bg-background/60 p-0.5"
                            >
                                <button
                                    v-for="tab in [
                                        { key: 'all', label: 'All' },
                                    ...
                                    @click="
                                        filterStatus =
                                            tab.key as typeof filterStatus
                                    "
                            ...
                            <div class="relative">
                                <select
                                    v-model="sortBy"
                                    class="appearance-none rounded-xl border border-border/40 bg-background/60 py-2.5 pr-8 pl-3 text-[11px] font-bold text-muted-foreground transition-all outline-none focus:border-primary/30 focus:ring-2 focus:ring-primary/10"
                                >
                                ...
                                <ArrowUpDown
                                    class="pointer-events-none absolute top-1/2 right-2.5 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground/40"
                                />
                            </div>
                        </div>
                    </div>
                ...
                >
                    ...
                    >
                        ...
                        >
                            <!-- Cover Image -->
                            <div
                                class="relative h-44 overflow-hidden bg-gradient-to-br from-primary/20 via-primary/10 to-muted"
                            >
                                <img
                                    v-if="course.cover_photo"
                                    :src="course.cover_photo"
                                ...
                                >
                                    <BookOpen
                                        class="h-16 w-16 text-primary/20"
                                    />
                                </div>
                                <!-- Overlay gradient -->
                                <div
                            ...
                            <div class="flex flex-1 flex-col p-5">
                                <h3
                                    class="text-lg font-bold tracking-tight transition-colors duration-500 group-hover/card:text-primary"
                                >
                                    {{ course.name }}
                                </h3>
                                <p
                                    v-if="course.description"
                                    class="mt-1.5 line-clamp-2 text-xs leading-relaxed text-muted-foreground/60"
                                >
                                    {{ course.description }}
                                </p>
                                ...
                                >
                                    ...
                                    <ChevronRight
                                        class="h-4 w-4 text-muted-foreground/40 transition-all duration-500 group-hover/card:translate-x-1 group-hover/card:text-primary"
                                    />
                                </div>
                ...
                >
                    <div
                        class="mb-6 flex h-20 w-20 items-center justify-center rounded-3xl border border-dashed border-border/40 bg-muted/10"
                    >
                        <Search class="h-8 w-8 text-muted-foreground/30" />
                    </div>
                    <h3 class="text-lg font-bold text-muted-foreground/60">
                        No matches found
                    </h3>
                ...
                >
                    <div
                        class="mb-6 flex h-24 w-24 items-center justify-center rounded-3xl border border-dashed border-border/40 bg-muted/10"
                    >
                        <BookOpen class="h-10 w-10 text-muted-foreground/30" />
                    </div>
                    <h3 class="text-xl font-bold text-muted-foreground/60">
                        No courses yet
                    </h3>
                    <p class="mt-2 text-sm text-muted-foreground/40">
                        Your enrolled courses will appear here once your
                        instructor assigns them.
                    </p>
                </Motion>
            </div>
        </template>
