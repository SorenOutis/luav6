...
const widths = ['60%', '75%', '50%', '85%', '65%', '55%', '80%', '70%'] as const;
...
<template>
    ...
    >
        ...
        <div v-if="hero" class="flex flex-col gap-4">
            <div class="flex items-start justify-between">
                <div class="flex flex-col gap-3">
                    <div class="h-6 w-56 animate-pulse rounded-md bg-primary/10"></div>
                    <div v-if="subtitle" class="h-4 w-80 animate-pulse rounded-md bg-primary/10"></div>
                </div>
            ...
            <div class="flex flex-wrap gap-3">
                <div v-for="p in 3" :key="p" class="h-8 w-20 animate-pulse rounded-lg bg-primary/10"></div>
            </div>
        </div>

        <!-- Stats Row -->
        <div v-if="stats > 0" class="grid grid-cols-2 gap-3 sm:grid-cols-3" :class="statsGridClass">
            <div
                v-for="s in stats"
                :key="s"
            ...
            >
                ...
                <div class="h-2 w-full animate-pulse rounded bg-primary/10"></div>
            </div>
        </div>
        ...
        <div v-if="variant === 'default'" class="flex flex-col gap-3">
            ...
            >
                <div v-if="cards.avatar" class="h-10 w-10 animate-pulse rounded-full bg-primary/10"></div>
                <div class="flex flex-1 flex-col gap-2">
                    <div class="h-4 animate-pulse rounded bg-primary/10" :style="{ width: widths[i % widths.length] }"></div>
                    <div
                ...
                <div v-if="cards.badge" class="h-6 w-16 animate-pulse rounded-md bg-primary/10"></div>
            </div>
        </div>
        ...
        <!-- Cards: grid of cards -->
        <div v-if="variant === 'cards'" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div
                v-for="i in count"
                :key="i"
            ...
            >
                <div v-if="cards.badge" class="h-5 w-16 animate-pulse rounded bg-primary/10"></div>
                <div class="h-5 animate-pulse rounded bg-primary/10" :style="{ width: cardsWidths[i % cardsWidths.length] }"></div>
                <div
                    v-for="l in cards.lines"
                    :key="l"
                    class="h-3 animate-pulse rounded bg-primary/10"
                    :style="{ width: cardsWidths[(i + l + 1) % cardsWidths.length] }"
                ></div>
            </div>
        </div>
        ...
        <div v-if="variant === 'list'" class="flex flex-col gap-1">
            ...
            >
                <div class="flex h-8 w-8 items-center justify-center">
                    <div class="h-5 w-5 animate-pulse rounded bg-primary/10"></div>
                </div>
                <div class="flex flex-1 flex-col gap-1.5">
                    <div class="h-4 animate-pulse rounded bg-primary/10" :style="{ width: widths[i % widths.length] }"></div>
                    <div class="h-3 animate-pulse rounded bg-primary/10" :style="{ width: widths[(i + 2) % widths.length] }"></div>
                </div>
                <div class="h-6 w-14 animate-pulse rounded-md bg-primary/10"></div>
            </div>
        </div>
        ...
        <div v-if="variant === 'hero-list'" class="flex flex-col gap-6">
            ...
            <div class="flex gap-6 border-b border-border/10 pb-3">
                <div v-for="t in 3" :key="t" class="h-5 w-20 animate-pulse rounded bg-primary/10"></div>
            </div>
            <div class="flex flex-col gap-3">
                <div
                    v-for="i in count"
                ...
                >
                    <div class="flex flex-1 flex-col gap-2">
                        <div class="h-4 animate-pulse rounded bg-primary/10" :style="{ width: widths[i % widths.length] }"></div>
                        <div class="h-3 animate-pulse rounded bg-primary/10" :style="{ width: widths[(i + 3) % widths.length] }"></div>
                    </div>
                    <div class="h-8 w-20 animate-pulse rounded-lg bg-primary/10"></div>
                </div>
            </div>
        </div>
    </div>
