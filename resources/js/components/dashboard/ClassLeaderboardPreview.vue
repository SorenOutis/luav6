...
onMounted(() => {
    ...
    const savedIndex = props.sectionLeaderboards.findIndex(
        (section) => section.sectionId === Number(saved),
    );
...
const activeLeaderboard = computed(
    () => props.sectionLeaderboards[activeIndex.value] ?? null,
);
const topUsers = computed(
    () => activeLeaderboard.value?.users.slice(0, 5) ?? [],
);
const currentUser = computed(
    () =>
        activeLeaderboard.value?.users.find((user) => user.isCurrentUser) ??
        null,
);
...
<template>
    <SpotlightCard
        customSize
        glowColor="blue"
        className="overflow-hidden p-4 sm:p-6 !bg-card/40"
        as="section"
    >
        <div
        ...
        >
            <div>
                <div class="mb-2 flex items-center gap-2">
                    ...
                    <span
                        class="text-[10px] font-black tracking-[0.22em] text-primary uppercase"
                    >
                        Class leaderboard
                    </span>
                </div>
                ...
                <p
                    v-if="activeLeaderboard"
                    class="mt-1 text-xs text-muted-foreground"
                >
                    You are
                    <span class="font-black text-foreground"
                        >#{{ activeLeaderboard.userRank || '-' }}</span
                    >
                    of {{ activeLeaderboard.totalPlayers }} classmates
                    <span v-if="activeSeasonName"
                        >in {{ activeSeasonName }}</span
                    >
                </p>
            </div>
            ...
            >
                <button
                    ...
                    type="button"
                    :class="[
                        'shrink-0 rounded-xl border px-3 py-2 text-[10px] font-black tracking-widest uppercase transition-all',
                        activeIndex === index
        ...
        <Transition name="hub-fade" mode="out-in">
            <div
                :key="
                    activeLeaderboard && topUsers.length > 0
                        ? 'content'
                        : 'empty'
                "
                ...
                <div v-else class="grid gap-3 lg:grid-cols-5">
                    ...
                    >
                        <div
                            ...
                            class="absolute -top-8 -right-8 h-20 w-20 rounded-full bg-primary/15 blur-2xl"
                            aria-hidden="true"
                        />

                        <div
                            class="relative z-10 flex items-center gap-3 lg:flex-col lg:text-center"
                        >
                            <div
                                :class="[
                            ...
                            <div
                                class="relative h-11 w-11 shrink-0 overflow-hidden rounded-xl border border-border/40 bg-muted/30 lg:h-14 lg:w-14"
                            >
                                <img
                                    v-if="user.avatar"
                                    :src="user.avatar"
                                    :alt="
                                        user.blurred ? 'Hidden user' : user.name
                                    "
                            ...
                            <div class="min-w-0 flex-1 lg:w-full">
                                <div
                                    class="flex items-center gap-1.5 lg:justify-center"
                                >
                                    <p
                                        class="lb-blurred-text truncate text-sm font-black tracking-tight group-hover:text-primary"
                                    >
                                        {{
                                            user.blurred
                                                ? BLURRED_NAME
                                    ...
                                    <span
                                        v-if="user.isCurrentUser"
                                        class="rounded-full bg-primary px-1.5 py-0.5 text-[7px] font-black text-primary-foreground uppercase"
                                    >
                                        You
                                    </span>
                                </div>
                                <div
                                    class="mt-1 flex items-center gap-2 text-[10px] font-bold text-muted-foreground lg:justify-center"
                                >
                                    <span
                                        >{{ user.xp.toLocaleString() }} XP</span
                                    >
                                    <span
            ...
            <div class="flex min-w-0 items-center gap-3">
                ...
                <p class="truncate text-xs font-bold">
                    You are currently ranked #{{
                        activeLeaderboard?.userRank
                    }}
                    with {{ currentUser.xp.toLocaleString() }} XP.
                </p>
            </div>
            <span
                class="shrink-0 text-[10px] font-black tracking-widest text-primary uppercase"
            >
                Keep climbing
            </span>
        </div>
