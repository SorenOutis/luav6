...
interface UserProgress {
    ...
    attempts: number;
    quizAnswers: Array<{ questionIndex: number; selectedOptionIndex: number }> | null;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.lesson.title, href: `/courses/${props.course.id}/lessons/${props.lesson.id}` },
];
...
const readingTime = computed(() => {
    ...
    const words = props.lesson.content.replace(/<[^>]*>/g, '').split(/\s+/).filter(Boolean).length;
...
const allQuestionsAnswered = computed(() => {
    if (!props.quiz) return false;
    return props.quiz.questions.every((_, i) => selectedAnswers.value[i] !== undefined);
});
...
const submitQuiz = () => {
    ...
    const answers = Object.entries(selectedAnswers.value).map(([questionIndex, selectedOptionIndex]) => ({
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="relative flex h-full flex-1 flex-col overflow-hidden bg-background">
                ...
                <template v-for="(mod, mIdx) in modules" :key="mod.id">
                    ...
                    <div class="shrink-0">
                        ...
                        <div class="hidden items-center gap-1 px-1 md:flex">
                            ...
                            <span
                                v-if="mod.lessons.every(l => l.completed) && mod.lessons.length > 0"
            ...
            <!-- ─── Top Bar ─── -->
            <div class="flex items-center justify-between border-b border-border/10 px-4 py-2.5 md:px-6">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[11px] font-medium text-muted-foreground/60">
                        Lesson {{ lessonNumber }} of {{ totalLessonsInCourse }}
                    </span>
                </div>
                <div class="flex items-center gap-3">
                    <span class="flex items-center gap-1.5 text-[11px] text-muted-foreground/50">
                        <Clock class="h-3.5 w-3.5" />
                        <span class="hidden sm:inline">{{ readingTime }}</span>
                    </span>
                    <span
                        class="rounded-full px-2.5 py-0.5 text-[10px] font-bold"
                        :class="userProgress.completed
                    ...
                    <div class="mx-auto max-w-4xl px-5 py-8 md:px-10 md:py-12">
                        ...
                        >
                            <div class="flex flex-wrap items-start justify-between gap-4">
                                <div class="flex-1">
                                    <h1 class="text-3xl font-black tracking-tight">{{ lesson.title }}</h1>
                                    <p class="mt-1.5 flex items-center gap-2 text-xs text-muted-foreground/50">
                                        <Clock class="h-3.5 w-3.5" />
                                        {{ readingTime }}
                                        <span class="mx-1.5 text-muted-foreground/20">·</span>
                                        <span>{{ userProgress.completed ? 'Completed' : 'Not yet completed' }}</span>
                                    </p>
                                </div>
                                <span class="flex items-center gap-1 rounded-full border border-border/30 bg-muted/30 px-3 py-1 text-[10px] font-medium text-muted-foreground md:hidden">
                                    <Clock class="h-3 w-3" />
                                    {{ readingTime }}
                                </span>
                            </div>
                            ...
                            >
                                ...
                                <div class="mb-6 flex items-center justify-between">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h2 class="text-lg font-bold">
                                                {{ userProgress.completed ? 'Lesson Complete!' : 'Check Your Understanding' }}
                                            </h2>
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span v-if="userProgress.completed" class="rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-bold text-emerald-500">
                                            <CheckCheck class="mr-1 inline h-3.5 w-3.5" />
                                            Completed
                                        </span>
                                        <span v-else-if="userProgress.attempts > 0" class="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">
                                            Attempt {{ userProgress.attempts }}{{ quiz.allowedAttempts > 0 ? `/${quiz.allowedAttempts}` : '' }}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div
                                    v-if="canAttemptQuiz && totalQuestions > 0"
                                    class="mb-6"
                                >
                                    <div class="flex items-center justify-between text-xs">
                                        ...
                                        <span class="font-bold" :class="allQuestionsAnswered ? 'text-emerald-500' : 'text-muted-foreground/50'">
                                            {{ allQuestionsAnswered ? '✓ Ready' : `${totalQuestions - answeredCount} remaining` }}
                                        </span>
                                    </div>
                                    <div class="mt-2 flex gap-1.5">
                                        <div
                                            ...
                                            :class="selectedAnswers[qIdx - 1] !== undefined
                                                ? 'bg-primary'
                                                : 'bg-muted'"
                                        ></div>
                                    </div>
                                ...
                                >
                                    <div class="flex items-start gap-3">
                                        <CheckCircle2 v-if="quizResult.passed" class="mt-0.5 h-5 w-5 shrink-0 text-emerald-500" />
                                        <AlertCircle v-else class="mt-0.5 h-5 w-5 shrink-0 text-red-500" />
                                        <div>
                                            <p class="text-sm font-bold" :class="quizResult.passed ? 'text-emerald-600' : 'text-red-600'">
                                                {{ quizResult.passed ? 'Congratulations! You passed!' : 'Not quite — keep studying and try again.' }}
                                            </p>
                                            <p class="mt-1 text-xs text-muted-foreground">
                                                Score: {{ quizResult.correctAnswers }}/{{ quizResult.totalQuestions }} ({{ quizResult.score }}%) &mdash; Required: {{ quizResult.passScore }}%
                                            </p>
                                        </div>
                                    </div>
                                </Motion>
                                ...
                                <!-- Quiz Questions -->
                                <div v-if="canAttemptQuiz" class="space-y-8">
                                    <div
                                    ...
                                    >
                                        <p class="text-sm font-semibold">
                                            <span class="text-muted-foreground/50">{{ qIdx + 1 }}.</span>
                                            {{ question.question }}
                                        </p>
                                        <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                                            <button
                                                ...
                                                :key="oIdx"
                                                @click="selectedAnswers[qIdx] = oIdx"
                                            ...
                                            >
                                                ...
                                                >
                                                    <div v-if="selectedAnswers[qIdx] === oIdx" class="h-2 w-2 rounded-full bg-current"></div>
                                                </div>
                                    ...
                                    <div class="flex items-center justify-between pt-4">
                                        <p v-if="!allQuestionsAnswered" class="text-xs text-muted-foreground/60">
                                        ...
                                        <button
                                            ...
                                            :class="allQuestionsAnswered && !isSubmitting
                                ...
                                <!-- Already Completed -->
                                <div v-if="userProgress.completed" class="flex flex-col items-center justify-center py-8">
                                    <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-emerald-500/10">
                                        <CheckCircle2 class="h-8 w-8 text-emerald-500" />
                                    </div>
                                    <p class="text-lg font-bold">You've completed this lesson!</p>
                                    <p class="mt-1 text-sm text-muted-foreground">
                                        Score: {{ userProgress.quizScore }}%
                                        <span v-if="userProgress.completedAt">· {{ new Date(userProgress.completedAt).toLocaleDateString() }}</span>
                                    </p>
                                </div>
                            </div>
                        </Motion>
                        ...
                        <div v-if="!nextLessonId" class="mt-8 flex justify-center">
                            <Link
                                :href="`/courses/${course.id}`"
...
:deep(.prose ol) {
    @apply mb-4 list-decimal pl-6 space-y-1;
}
:deep(.prose li) {
...
:deep(.prose pre) {
    @apply rounded-xl bg-muted p-4 overflow-x-auto my-4;
}
:deep(.prose a) {
