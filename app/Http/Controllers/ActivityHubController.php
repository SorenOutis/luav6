...
{
    /**
     * Activities Hub — unified view of Exams, Assignments, and Courses.
     *
     * Minimalist hub with 3 core types inside tabs. Mobile-first.
     * Reuses existing listing logic from ExamController, AssignmentController,
     * and CourseController to avoid drift.
     */
    public function index(Request $request): Response
    {
        ...
        $sectionTabs = collect([
            ...
            ->merge(
                $sectionNames->map(function ($name) use (
                    $allExams,
                    $assignmentsPayload,
                ) {
                    $examCount = $allExams
        ...
        return Inertia::render('Activities/Index', [
            ...
            'hubStats' => [
                'total' => $totalCount,
                'pending' =>
                    $pendingExams + $pendingAssignments + $pendingCourses,
                'completed' =>
                    $completedExams + $completedAssignments + $completedCourses,
                'exams' => [
    ...
    /**
     * Keep exam listing logic identical to ExamController to avoid drift.
     *
    ...
    private function examPage(User $user, ?string $cursor = null): array
    {
        ...
        $examsData = $exams->map(function (Exam $exam) use ($allSubmissions) {
            ...
            $submittedPartsCount = $submissions
                ->unique('exam_part_id')
                ->count();
            ...
            return array_merge($exam->withoutRelations()->toArray(), [
                ...
                'is_locked' =>
                    ($submittedPartsCount === $exam->parts->count() &&
                        $exam->parts->isNotEmpty()) ||
                    $exam->status === 'closed',
                'has_submissions' => $submissions->isNotEmpty(),
                'results_available' =>
                    $exam->status === 'closed' && $submissions->isNotEmpty(),
        ...
        $seasonRank = Season::query()
            ->whereIn(
                'id',
                $exams->pluck('section.season_id')->filter()->unique(),
            )
                ...
                return [
                    ...
                    'group' => $group
                        ? [
                            ...
                            'members' => $group->members
                                ...
                                ->values(),
                            'pending_invites' => $pendingByGroup->has(
                                $group->id,
                            )
                                ? $pendingByGroup
                                    ->get($group->id)
                    ...
                    'submission' => $submission
                        ? [
                            ...
                            'submitted_by_name' => $uploaders->get(
                                $pivot->submitted_by,
                            )?->name,
                            ...
                            'has_unseen_feedback' => $this->hasUnseenFeedback(
                                $pivot,
                            ),
    ...
    private function hasUnseenFeedback(?Submission $submission): bool
    {
        ...
        return $submission->graded_at !== null &&
            $submission->feedback_seen_at->lt($submission->graded_at);
    }
        ...
        return $user
            ...
            ->map(function ($course) {
                return [
                    ...
                    'completedLessons' =>
                        $course->pivot->completed_lessons ?? 0,
                    'progress' =>
    ...
    private function buildUnifiedTimeline($exams, $assignments, $courses): array
    {
        ...
        foreach ($assignments as $assignment) {
            ...
            $isCompleted =
                (bool) ($assignment['submission']['submitted'] ?? false);
            $items->push([
                'kind' => 'assignment',
                'id' => $assignment['id'],
                'title' => $assignment['title'],
    ...
    public function listing(Request $request): JsonResponse
    {
        return response()->json(
            $this->examPage($request->user(), $request->query('cursor')),
        );
    }
}
